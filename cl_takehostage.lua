...
--TakeHostage by Robbster, do not redistrbute without permission--
	...
	RegisterNetEvent('takeHostage:takeHostage')
	AddEventHandler('takeHostage:takeHostage', function()
		takeHostage()
	end)
	...
	function takeHostage()
		...
		if not canTakeHostage then 
			drawNativeNotification("Je moet een pistool vasthouden!")
		end

		local vehicle = GetVehiclePedIsUsing(GetPlayerPed(-1))

		if vehicle ~= nil then
			--drawNativeNotification("Dit kan niet als je in een voertuig zit.")
	...
	RegisterNetEvent('Maniusdevelopement:syncTarget')
	AddEventHandler('Maniusdevelopement:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
	...
	RegisterNetEvent('Maniusdevelopement:syncMe')
	AddEventHandler('Maniusdevelopement:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
		local playerPed = GetPlayerPed(-1)
	...
	RegisterNetEvent('Maniusdevelopement:cl_stop')
	AddEventHandler('Maniusdevelopement:cl_stop', function()
		holdingHostageInProgress = false
		beingHeldHostage = false 
		...
		while true do 
			if holdingHostage then
				if IsEntityDead(GetPlayerPed(-1)) then
					...
					local closestPlayer = GetClosestPlayer(2)
					target = GetPlayerServerId(closestPlayer)
					TriggerServerEvent("Maniusdevelopement:stop",target)
					Wait(100)
					releaseHostage()
				end 
	...
	function releaseHostage()
		...
		target = GetPlayerServerId(closestPlayer)
		if closestPlayer ~= nil then
			print("[MANIUS DEVELOPEMENT]")
			TriggerServerEvent('Maniusdevelopement:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
			print("[MANIUS DEVELOPMENT] No player nearby")
		end
	end 
	...
	function killHostage()
		...
		target = GetPlayerServerId(closestPlayer)
		if closestPlayer ~= nil then
			print("triggering Maniusdevelopement:sync")
			TriggerServerEvent('Maniusdevelopement:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
			print("[CMG Anim] No player nearby")
		end	
